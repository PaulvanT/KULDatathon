...
df = make_dataframe(data_test)
export_to_csv(id, df)"""

def make_usable_dataframe(id):
  data = request_data_reports(id)
  df = make_dataframe(data, id)
  return df
...
#id_list = [349547,24948, 26408, 26940,9000000037,27672, 27740, 27742, 27749]
#id_list = [349547]
df_test = get_all_data(id_list)

df_test = df_test[~((df_test.bucket0 == 0) & (df_test.bucket1 == 0) & (df_test.bucket2 == 0) & (df_test.bucket3 == 0) & (df_test.bucket4 == 0) & (df_test.bucket5 == 0) & (df_test.bucket6 == 0) & (df_test.bucket7 == 0))]
#df_test.to_csv('ALL_DATA.csv', sep=',')
#df_test.to_csv('ALL_DATA.csv', sep=',')

print(len(pd.read_csv('ALL_DATA.csv')))
